...
L=(35,50,t0,t0+50.,0.,2*pi,0.,1.,0.,Kmax,np.min(Vobs),np.max(Vobs))
...
ubest,chi2=algorithme_genetique(vkepler,6,+1,L,(0.1,0.9,100,2000))
#    chiff.append(chi2)
...
for i in range(0,len(tjj)) :
        ...
        v=2*np.arctan(np.sqrt((1+e)/(1-e))*np.tan(E/2))
        V[i]=V0+k*(np.cos(w+v))+e*np.cos(w)
plt.plot(tjj,V,label='fit')
plt.errorbar(tj,Vobs,yerr=sigma,fmt='.',label='Observée')
plt.xlabel('J.D')
