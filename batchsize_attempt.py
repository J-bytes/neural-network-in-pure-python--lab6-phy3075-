...
def innitialisation() :
    ...
    global gradih,gradhn,gradno
    ni,nh,nn,no    =13,6,4,1        # nombre d’unites d’entree, interne et de sortie
    ...
    gradih   =np.random.uniform(-0.5,0.5,[ni,nh])   # poids des connexions entree vers interne
    gradhn   =np.random.uniform(-0.5,0.5,[nh,nn])   # poids des connexions interne vers sortie
    gradno   =np.random.uniform(-0.5,0.5,[nn,no])   # poids des connexions interne vers sortie
...
def gradient(param):
    ...
    gradno[:,:]+=eta*np.outer(deltao[:],sn[:]).transpose() # Eq. (6.17) pour les wHO
    bno[:,:]+=eta*deltao[:]
    sum=np.sum((deltao[:]*wno[:,:]),axis=1)
    deltan[:]=dactv(sn[:])*sum           # Eq. (6.21)
        
    gradhn[:,:]+=eta*np.outer(deltan[:],sh[:]).transpose() # Eq. (6.17) pour les wIH pour les wIH
    bhn[:,:]+=eta*deltan[:]
    sum=np.sum((deltan[:]*whn[:,:]),axis=1)
    deltah[:]=dactv(sh[:])*sum           # Eq. (6.21)
        
    gradih[:,:]+=eta*np.outer(deltah[:],ivec[:]).transpose() # Eq. (6.17) pour les wIH pour les wIH
    bih[:,:]+=eta*deltah[:]
    return 
...
def backprop(param):
    ...
    wno[:,:]+=gradno/batch_size # Eq. (6.17) pour les wHO
   
    whn[:,:]+=gradhn/batch_size # Eq. (6.17) pour les wIH pour les wIH
   
    wih[:,:]+=gradih/batch_size # Eq. (6.17) pour les wIH pour les wIH
...
def training(param) :
    ...
    for iter in range(0,niter):       # boucle sur les iteration d’entrainement
            ...
            for itrain in range(0,nset,batch_size):
               
                for j in range(0,batch_size,1):   # boucle sur l’ensemble d’entrainement
                    
                    itt=rvec[itrain+j]            # le membre choisi...
...
def prediction(param) :
    ...
    nset=200 #nombre d'événement tester
