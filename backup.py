...
#Variables globales
def innitialisation(nh) :
   
   
    ni,no    =13,1        # nombre d’unites d’entree, interne et de sortie
...
def prediction(param) :
    ...
    for i in range(0,nset) :
        ivec=tset[i,:]
        wih,who,ni,no,ivec,sh,so,err,deltao,deltah,eta,nh=ffnn([wih,who,ni,no,ivec,sh,so,err,deltao,deltah,eta,nh])
        reponse[i]=so
    return reponse,cheatsheet

for nh in [20] :
    param=innitialisation(nh)
