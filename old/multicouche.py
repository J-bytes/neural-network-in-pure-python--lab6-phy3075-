...
def setup(nn) :
    ...
    wih   =np.zeros((ni,nh,nn))   # poids des connexions entree vers interne
    who   =np.zeros((nh,no,nn))  # poids des connexions interne vers sortie
    ivec  =np.zeros(ni)        # signal en entree
    sh    =np.zeros((nh,nn))        # signal des neurones interne
...
def ffnn(ivec,nh,actv,nn):
         # couche d’entree a couche interne
    sh[:,0]+=actv( np.sum(wih[:,:,0]*np.array([ivec[:],]*8).transpose(),axis=0))        # Eq. (6.1)        # Eq. (6.2)
    
    sh[:,:]+=actv( np.sum(wih[:,:,:]*wih[:,:,:],axis=0))
    # couche interne a couche de sortie
    shtemp=np.array([sh[:,nn-1],]*no).transpose()    
    # Eq. (6.3) avec b_1=0
    so[:]+=actv(np.sum(who[:,:,nn-1]*shtemp[:,:],axis=0))              # Eq. (6.4)
    return 
#ENDfonctionffnn
#----------------------------------------------------------------------
#retropropagation du signal d’erreur et ajustement des poids du reseau
def backprop(err,nh,dactv,nn):
    """
    ...
    for io in range(0,no): # couche de sortie a couche interne
        ...
        for ih in range(0,nh):           # couche interne a couche de sortie
            sum=np.sum(deltao[:]*who[ih,:,ih2])
            deltah[ih,ih2]=dactv(sh[ih,ih2])*sum           # Eq. (6.21)
            
            wih[:,ih,ih2]+=eta*deltah[ih,ih2]*ivec[:] # Eq. (6.17) pour les wIH pour les wIH
              # couche interne a couche de sortie
    for ih2 in range(1,nn) :
         for ih in range(0,nh):
            sum=np.sum(deltao[:]*who[ih,:,ih2])
            deltah[ih,ih2]=dactv(sh[ih,ih2])*sum           # Eq. (6.21)
            
            wih[:,ih,ih2]+=eta*deltah[ih,ih2]*wih[:,ih,ih2-1] # Eq. (6.17) pour les wIH pour les wIH
    return
#ENDfonctionbackpro
...
def training(actv,dactv,batch_size) :
    ...
    nn=2 #?????
    wih[:,:,:]=np.random.uniform(-0.5,0.5,size=(ni,nh,nn)) # poids entree-interne
    who[:,:,:]=np.random.uniform(-0.5,0.5,size=(nh,no,nn)) # poids interne-sortie
    ...
    for iter in range(0,niter):       # boucle sur les iteration d’entrainement
            ...
            for iter2 in range(0,nset,batch_size):   # boucle sur l’ensemble d’entrainement
                ...
                sh[:]/=batch_size
                so[:]/=batch_size
                backprop(err,nh,dactv,nn) # retropropagation          # retropropagation
    #ENDbouclesurensemble
            ...
            #ENDMAIN
    plt.plot(rmserr,'.',label='batch_size'+str(batch_size))
