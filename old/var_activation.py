...
def ffnn(ivec,nh,actv):
    ...
    sh[:]=actv( np.sum(np.dot(wih[:,:].transpose(),ivec[:]),axis=0))        # Eq. (6.1)        # Eq. (6.2)
    # couche interne a couche de sortie
    
    # Eq. (6.3) avec b_1=0
    so[:]=actv(np.sum((who[:,:]*sh[:].reshape(nh,no)),axis=0))              # Eq. (6.4)
    return 
#ENDfonctionffnn
#----------------------------------------------------------------------
...
def randomize(n):
    dumvec=np.zeros(n)
    for k in range(0,n):
        dumvec[k]=np.random.uniform() # tableau de nombres aleatoires
...
def training(actv,dactv) :
    ...
    nset  =4                 # nombre de membres dans ensemble d’entrainement
...
for actv,dactv in zip([sigmoide],[dsigmoide]) :
    setup()
    training(actv,dactv)
