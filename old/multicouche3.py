...
#Variables globales
def setup() :
    global wih,wno,ni,no,ivec,sh,so,err,deltao,deltah,eta,nh,deltan
    global whn,sn,deltan,nn
    ...
    whn   =np.zeros((nh,nn))
    wno   =np.zeros([nh,no])   # poids des connexions interne vers sortie
...
def ffnn(ivec,nh,actv):
    ...
    sh[:]=actv( np.sum(wih[:,:]*np.array([ivec[:],]*nh).transpose(),axis=0))        # Eq. (6.1)        # Eq. (6.2)
        
    shtemp=np.array([sh,]*nn).transpose()       
    #couche interne à couche interne 
    sn[:]=actv(np.sum(whn[:,:]*shtemp[:,:],axis=0))
    
    # couche interne a couche de sortie
    sntemp=np.array([sh,]*no).transpose()    
    # Eq. (6.3) avec b_1=0
    so[:]=actv(np.sum(wno[:,:]*sntemp[:,:],axis=0))              # Eq. (6.4)
    return 
#ENDfonctionffnn
#----------------------------------------------------------------------
...
def backprop(err,nh,dactv):
   ...
   # couche de sortie a couche interne
    ...
    wno[:,:]+=eta*np.dot(deltao[:],np.array([sn[:],])).reshape(nn,no) # Eq. (6.17) pour les wHO
    
    #couche interne à couche interne
    sum=np.sum(deltao[:]*wno[:,:],axis=0)
    deltan[:]=dactv(sn[:])*sum           # Eq. (6.21)
        
    whn[:,:]+=eta*np.dot(np.array([deltan[:],]).transpose(),np.array([sh[:],])).reshape(nh,nn) # Eq. (6.17) pour les wIH pour les wIH
    
    # couche interne a couche d'entrée
        
    sum=np.sum(deltan[:]*whn[:,:],axis=0)
    deltah[:]=dactv(sh[:])*sum           # Eq. (6.21)
        
    wih[:,:]+=eta*np.dot(np.array([deltah[:],]).transpose(),np.array([ivec[:],])).reshape(ni,nh) # Eq. (6.17) pour les wIH pour les wIH
    return 
#ENDfonctionbackpro
