...
def training(param) :
    wih,whn,wno,ni,nh,nn,no,ivec,sh,so,sn,err,deltao,deltah,deltan,eta,nh,bih,bhn,bno=param
    nset  =40000               # nombre de membres dans ensemble d’entrainement
    ...
    tset=pd.read_csv('training2.csv',header=1,delimiter=',').values[0:nset,1:14]                          # diverses instructions...
    reponse=pd.read_csv('training2.csv',header=1,delimiter=',').values[0:nset,32]
    for i in range(0,nset) :
        oset[i,0]=np.int(reponse[i][0])
    tset=np.array(tset,dtype=np.float32)
...
def prediction(param) :
    nset=4000 #nombre d'événement tester
    r=np.random.randint(0,20000,size=nset)
    oset  =np.zeros(nset)
    wih,whn,wno,ni,nh,nn,no,ivec,sh,so,sn,err,deltao,deltah,deltan,eta,nh,bih,bhn,bno=param
    tset=pd.read_csv('training2.csv',header=1,delimiter=',').values[r,1:14]                          # diverses instructions...
    reponse=pd.read_csv('training2.csv',header=1,delimiter=',').values[r,32]
    tset=np.array(tset,dtype=np.float32)
    for i in range(0,nset) :
        oset[i]=np.int(reponse[i][0])                   # diverses instructions...
